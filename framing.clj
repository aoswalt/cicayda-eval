...
(defn frameStrings [stringList frameChar]
  ...
  (def cap
    (str frameChar frameChar
         (apply str (repeat longestLength frameChar))
         frameChar frameChar))
  (def body
    (map
      #(str frameChar " " (stringPad %1 longestLength) " " frameChar)
      stringList))
...
(println ["Hello" "World" "in" "a" "frame"])
(println (frame ["Hello" "World" "in" "a" "frame"]))
(println (str ["A" "different" "set" "of" "strings" "to" "test"] " with #"))
(println (frame ["A" "different" "set" "of" "strings" "to" "test"] "#"))
